...
router.post('/add-parent',upload.single('profilePics'),(req,res,next)=>{
  ...
  const checkEmail =  helper.validateEmail(email);
    ...
    Parent.findOne({$or:[{"email":email,"mobileNo":mobileNo}]}).then(result=>{
        if(result)
        {
            return res.status(200).json({
                'status':"0",
...
router.get('/parent-by-id/:id',(req,res,next)=>{
    ...
    Parent.findOne({_id:req.params.id}).then(result=>{
        return res.status(200).json({
            'status':"1",
            "data":result,
...
router.put('/update-parent/:id',upload.single('profilePics'),(req,res,next)=>{
    const {
        branchId,       
        name,
        relation,
    ...
    Parent.findOneAndUpdate({_id:req.params.id},{
        $set:{
            branchId,       
            name,
            relation,
...
router.delete('/delete-parent/:id',(req,res,next)=>{
    ...
    Parent.deleteOne({_id:req.params.id}).then(result=>{
        res.status(200).json({
            'status':"1",
            "data":result,
